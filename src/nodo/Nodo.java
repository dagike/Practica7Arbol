...
public class Nodo<T> {
	...
	public int compareTo(T cmp) {
		if (cmp.getClass() == String.class) {
			...
			if (Integer.valueOf(String.valueOf(info)) < Integer.valueOf(String
					.valueOf(cmp)))
				return 1;
			else
				return -1;
		} else if (cmp.getClass() == Double.class) {
			if (Double.valueOf(String.valueOf(info)) < Double.valueOf(String
					.valueOf(cmp)))
				return 1;
			else
				return -1;
		} else if (cmp.getClass() == NumeroComplejo.class) {
			if (((NumeroComplejo) info).getParteReal() < ((NumeroComplejo) cmp)
					.getParteReal()) {
				return 1;
			} else if (((NumeroComplejo) info).getParteReal() == ((NumeroComplejo) cmp)
					.getParteReal()
					&& ((NumeroComplejo) info).getParteImaginaria() < ((NumeroComplejo) cmp)
							.getParteImaginaria()) {
				return 1;
			} else
				return -1;
