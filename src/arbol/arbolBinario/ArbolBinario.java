...
public class ArbolBinario<T> extends Arbol<T>{
	...
	public Nodo<T> getRaiz(){return nodoDoble;}
	...
	public boolean eliminar(T info){//Pac
		if(info.equals(nodoDoble.getInfo())){
			...
			if(((NodoDoble<T>)temp2).getPreviewNodo() == temp){
				if(((NodoDoble<T>)temp).getPreviewNodo() == null && temp.getNextNodo() == null){//es una hoja
					((NodoDoble<T>)temp2).setPreviewNodo(null);
					return true;
				}
				
				if(((NodoDoble<T>)temp).getPreviewNodo() == null || temp.getNextNodo() == null){//tiene solo un hijo
					if(((NodoDoble<T>)temp).getNextNodo() == null)
						((NodoDoble<T>)temp2).setPreviewNodo(((NodoDoble<T>)temp).getPreviewNodo());
					else
					...
					Nodo<T> temp3 = temp;
					while(((NodoDoble<T>)temp3).getPreviewNodo() != null && temp3.getNextNodo() != null){
					...
					((NodoDoble<T>)temp3).setPreviewNodo(((NodoDoble<T>)temp).getPreviewNodo());
					return true;
				}	
			}
				...
				if(((NodoDoble<T>)temp).getPreviewNodo() == null || temp.getNextNodo() == null){
					if(((NodoDoble<T>)temp).getNextNodo() == null)
						temp2.setNextNodo(((NodoDoble<T>)temp).getPreviewNodo());
					else
